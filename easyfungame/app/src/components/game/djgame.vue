<template>
	<div>
		...
		<div class="newest">
			<ul>
				<li v-for="(item,index) in getNoticeGameData">
					<p>
						{{index+1}}
					</p>
					...
					<button class="button"><span  @click="xz(item.id,item.downUrl)" >下载</span></button>
				</li>
			</ul>
		</div>
	...
	export default {
		...
		methods: {
			xz(gameid, url1) {
				...
				$.ajax({
					type: "get",
					url: contextPath + "/liyu_game/api/gameinfo/downloadingstatistics?",
					async: true,
					data: {
...
<style scoped>
	...
	.newest ul li button a{
		display: block;
		color: white;
	}
	.newest ul li button span{
		display: block;
		color: white;
	}
