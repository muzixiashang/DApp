	...
	<div class="content">
		...
		<div class="payNumBox">
			...
			<div class="payment">
				...
				<label class="tip" v-if="payWay==1">提示:  TTC汇算1个工作日内到账</label>
			</div>
		...
		methods: {
			...
			buyTTC: function() {
				...
				if(vm.payWay == 0) {
					$.ajax({
						type: "post",
						url: contextPath + "/liyu_game/api/h5sdk/alipay/rechargeTTC",
						async: true,
						ataType: "json",
						data: {
							shoppingPrice:  0.3 ,
							shoppingNum: vm.customizedTTC,
							subject: "TTC游戏充值" + vm.customizedTTC,
					...
					mui.confirm("转入数量:" + vm.customizedTTC + "TTC</br>价值:" + vm.customizedTTC *  0.3  + "¥", "TTC数字资产转入到中国区鲤鱼轻游戏平台", ["取消", "确认"], function(e) {
						if(e.index == 1) {
							vm.$router.push({
								"path": "/ttctransfer",
								query: {
					...
					if(vm.isWx == 1) { //微信公众号
						$.ajax({
							type: "post",
							url: contextPath + "/liyu_game/api/sdkWxPay/getWxTTCPaySign",
							async: true,
							ataType: "json",
							data: {
								shoppingPrice:  0.3 ,
						...
						$.ajax({
							type: "post",
							url: contextPath + "/liyu_game/api/sdkWxPay/getH5TTCWxPaySign",
							async: true,
							dataType: "json",
							data: {
								shoppingPrice:  0.3 ,
			...
			submitPay: function() {
				...
				if($("#customizedTTC1").val() >= 1 && $("#customizedTTC1").val() != "" && $("#customizedTTC1").val() != undefined) {
					mui.confirm("支付金额:" + vm.customizedTTC *  0.3  + "元", "充值金额:" + vm.customizedTTC + "TTC", ["取消", "确认"], function(e) {
						if(e.index == 1) {
							vm.buyTTC();
						} else {
							mui.toast("订单已取消")
