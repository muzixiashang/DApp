        ...
        <div class="jyjl_list">
           ...
           <ul>
               <li @click="gototibidetails(item)" v-for="(item , index) in listshow" :key=index>
                   ...
                   <div class="list_r">
                       <h4>
                            <span v-if=" item.state == 0" class="sh">审核中...</span>
                       <span v-if="item.state == 1" class="cg">提币成功</span>
                       <span v-if="item.state == 2" class="bh">驳回</span>
...
export default {
  ...
  methods: {
    ...
    gettibi: function() {
      ...
      $.ajax({
        type: "get",
        url: contextPath + "/liyu_game/api/currency/findCurrencyHistory",
        async: true,
        data: {
