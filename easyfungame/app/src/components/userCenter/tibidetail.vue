    ...
    <div class="bg1">
        ...
        <div class="showdetails">
            <div class="showdetails_inner">
                ...
                <div class="tibi_bot" >
                    <p >提币金额&nbsp;:<em>{{newscon.ttcnum}}TTC</em></p>
                     <p v-if="newscon.state == 0">订单状态&nbsp;:<em class="shz">审核中</em></p>
                     <p v-if="newscon.state == 1">订单状态&nbsp;:<em class="tbcg">提币成功</em></p>
                     <p v-if="newscon.state == 2">订单状态&nbsp;:<em class="bh">驳回</em></p>
                    <p v-if="newscon.reason != ''">驳回理由&nbsp;:<em>{{newscon.reason}}</em></p>
                     <p >订单日期&nbsp;:<em>{{newscon.createtime}}</em></p>
                      <p >提币地址&nbsp;:<em>{{newscon.address}}</em></p>
                      <p >订单流水号&nbsp;:<em>{{newscon.ordernum}}</em></p>
...
export default {
  ...
  methods: {
    gettibidetails: function() {
      ...
      $.ajax({
        type: "get",
        url: contextPath + "/liyu_game/api/currency/findcurrencydetailsById?",
        async: true,
        data: {
